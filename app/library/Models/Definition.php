...
abstract class Definition extends Migration {
	...
	protected function _index(String $column_names, String $name=null, Boolean $is_unique=false) {
		if(!$name)
			$name = 'PRIMARY';
		return new Index($name, __strmap($column_names), $is_unique);
	}
	...
	protected function _refer(String $name, String $referee, String $referrer_column, String $referrer_table=null, String $referrer_schema=null) {
		$opts = [
			'columns' => __strmap($referee),
			'referencedColumns' => __strmap($referrer_column),
		];
		if($referrer_table)
